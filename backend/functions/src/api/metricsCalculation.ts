...
async function calculateSalesMetrics(tenantId: string, startDate: Date, endDate: Date) {
  ...
  salesSnapshot.forEach((doc: admin.firestore.QueryDocumentSnapshot) => {
    ...
    // Payment breakdown
    paymentBreakdown[sale.paymentMethod as keyof typeof paymentBreakdown] += sale.amount;
...
async function calculateExpenseMetrics(tenantId: string, month: string) {
  const db = admin.firestore();
...
async function calculateAdMetrics(tenantId: string, month: string) {
  const db = admin.firestore();
  ...
  const byMedium: Record<string, { adCost: number; conversions: number; cpa: number; roi: number }> = {};
  ...
  const averageROI = adsSnapshot.size > 0
    ? adsSnapshot.docs.reduce((sum: number, doc: admin.firestore.QueryDocumentSnapshot) => sum + (doc.data().roi || 0), 0) / adsSnapshot.size
    : 0;
...
async function calculateCustomerMetrics(tenantId: string) {
  const db = admin.firestore();
  ...
  customersSnapshot.forEach((doc: admin.firestore.QueryDocumentSnapshot) => {
    ...
    if (customer.courseStatus) {
      statusBreakdown[customer.courseStatus as keyof typeof statusBreakdown]++;
    }
  });
  ...
  const customersWithNextAppointment = new Set(
...
async function calculateMetrics(tenantId: string, date: string, period: "daily" | "weekly" | "monthly") {
  ...
  let noshowCount = 0;
  appointmentsSnapshot.forEach((doc: admin.firestore.QueryDocumentSnapshot) => {
    const apt = doc.data();
...
async function generateKPIAlerts(tenantId: string, metricsData: any, period: string) {
  ...
  if (kpiTargetsSnapshot.empty) return;
...
export const calculateDailyMetrics = onCall(
...
export const scheduledDailyMetrics = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      const promises = tenantsSnapshot.docs.map(async (tenantDoc: admin.firestore.QueryDocumentSnapshot) => {
...
export const scheduledWeeklyMetrics = onSchedule(
  {
    schedule: "0 3 * * 1",
    timeZone: "Asia/Tokyo",
    region: "asia-northeast1",
  ...
  async () => {
    ...
    try {
      const db = admin.firestore();
      const tenantsSnapshot = await db.collection("tenants").get();
      const lastMonday = format(subDays(new Date(), 7), "yyyy-MM-dd");
...
export const scheduledMonthlyMetrics = onSchedule(
  {
    schedule: "0 4 1 * *",
    timeZone: "Asia/Tokyo",
    region: "asia-northeast1",
  ...
  async () => {
    ...
    try {
      const db = admin.firestore();
      const tenantsSnapshot = await db.collection("tenants").get();
      const firstDayOfLastMonth = format(startOfMonth(subDays(new Date(), 1)), "yyyy-MM-dd");
...
// Get metrics
export const getMetrics = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    requireAuth(request);
    ...
    try {
      ...
      const metrics = snapshot.docs.map((doc: admin.firestore.QueryDocumentSnapshot) => ({
        id: doc.id,
        ...doc.data(),
      }));
