...
const getCustomerByLineUserIdSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
});

const getCustomerAppointmentsSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
  limit: z.number().optional().default(10),
});

const getCustomerChartsSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
  limit: z.number().optional().default(20),
});

const createCustomerAppointmentSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
...
const cancelCustomerAppointmentSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
  appointmentId: z.string(),
});

const registerLineCustomerSchema = z.object({
  lineUserId: z.string(),
  tenantId: z.string(),
 ...
 */
export const getCustomerByLineUserId = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    try {
      const { lineUserId, tenantId } = getCustomerByLineUserIdSchema.parse(
        request.data
      );
 ...
 * 顧客の予約一覧を取得
 */
export const getCustomerAppointments = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    try {
      const { lineUserId, tenantId, limit } =
        getCustomerAppointmentsSchema.parse(request.data);
      ...
      if (customerSnapshot.empty) {
        throw new HttpsError("not-found", "顧客情報が見つかりませんでした");
      }
 ...
 * 顧客のカルテ一覧を取得
 */
export const getCustomerCharts = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    try {
      const { lineUserId, tenantId, limit } = getCustomerChartsSchema.parse(
        request.data
      );
      ...
      const charts = await Promise.all(
        chartsSnapshot.docs.map(async (doc) => {
          const data = doc.data();
      ...
      throw new HttpsError(
        "internal",
        error.message || "Failed to get charts"
      );
    }
  }
);
 ...
 * 顧客が予約を作成（LIFF経由）
 */
export const createCustomerAppointment = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    try {
      const { lineUserId, tenantId, serviceIds, startAt, notes } =
        createCustomerAppointmentSchema.parse(request.data);
      ...
      for (const serviceId of serviceIds) {
        ...
        if (!serviceDoc.exists) {
          throw new HttpsError("not-found", `サービスが見つかりません: ${serviceId}`);
        }
      ...
      const endAtDate = admin.firestore.Timestamp.fromDate(
        new Date(new Date(startAt).getTime() + totalDuration * 60 * 1000)
      );
...
export const cancelCustomerAppointment = onCall(
  ...
  async (request) => {
    try {
      const { lineUserId, tenantId, appointmentId } =
        cancelCustomerAppointmentSchema.parse(request.data);
      ...
      if (!appointmentDoc.exists) {
        throw new HttpsError("not-found", "予約が見つかりませんでした");
      }
 ...
 */
export const registerLineCustomer = onCall(
  { region: "asia-northeast1", cors: true },
  async (request) => {
    try {
      const { lineUserId, tenantId, displayName, pictureUrl } =
        registerLineCustomerSchema.parse(request.data);
