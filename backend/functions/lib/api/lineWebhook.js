 ...
 * 署名検証とテナント識別
 */
async function verifySignatureAndGetTenant(body, signature) {
    ...
    for (const tenantDoc of tenantsSnapshot.docs) {
        ...
        // 署名検証
        if ((0, line_1.verifySignature)(body, signature, channelSecret)) {
            return tenantId;
        }
    }
    return null;
 ...
 * テキストメッセージの処理
 */
async function handleTextMessage(event, tenantId) {
    ...
    try {
        ...
        // コマンド処理
        if (text === "予約確認") {
            await handleReservationCheck(db, userId, tenantId, replyToken, lineConfig.channelAccessToken);
        }
        else if (text === "配信停止") {
            await handleUnsubscribe(db, userId, tenantId, replyToken, lineConfig.channelAccessToken);
        }
        else if (text === "再開" || text === "配信再開") {
            await handleResubscribe(db, userId, tenantId, replyToken, lineConfig.channelAccessToken);
        }
        else {
            // デフォルトメッセージ（任意）
            // 必要に応じて自動応答メッセージを実装
 ...
 * 友だち追加イベントの処理
 */
async function handleFollowEvent(event, tenantId) {
    const userId = event.source.userId;
    if (!userId) {
        return;
    }
    try {
        ...
        // ユーザープロフィールを取得
        const profile = await (0, line_1.getProfile)(userId, lineConfig.channelAccessToken);
        // 顧客データにLINE情報を保存
        const db = admin.firestore();
        ...
        await (0, line_1.pushMessage)(userId, [(0, line_1.createTextMessage)(welcomeMessage)], lineConfig.channelAccessToken);
    }
    catch (error) {
 ...
 * ブロック（友だち削除）イベントの処理
 */
async function handleUnfollowEvent(event, tenantId) {
    const userId = event.source.userId;
    if (!userId) {
        return;
    }
 ...
 * 予約確認コマンドの処理
 */
async function handleReservationCheck(db, userId, tenantId, replyToken, accessToken) {
    // 顧客情報を取得
    ...
    if (customersSnapshot.empty) {
        await (0, line_1.replyMessage)(replyToken, [(0, line_1.createTextMessage)("顧客情報が見つかりませんでした。")], accessToken);
        return;
    }
    ...
    if (appointmentsSnapshot.empty) {
        await (0, line_1.replyMessage)(replyToken, [(0, line_1.createTextMessage)("現在、予約はありません。")], accessToken);
        return;
    }
    // 予約リストを作成
    let message = "【ご予約一覧】\n\n";
    ...
    await (0, line_1.replyMessage)(replyToken, [(0, line_1.createTextMessage)(message)], accessToken);
}
/**
 * 配信停止コマンドの処理
 */
async function handleUnsubscribe(db, userId, tenantId, replyToken, accessToken) {
    const customersSnapshot = await db
        .collection(`tenants/${tenantId}/customers`)
        .where("lineUserId", "==", userId)
        .limit(1)
    ...
    await (0, line_1.replyMessage)(replyToken, [
        (0, line_1.createTextMessage)("配信を停止しました。\n再開する場合は「再開」と送信してください。"),
    ], accessToken);
}
/**
 * 配信再開コマンドの処理
 */
async function handleResubscribe(db, userId, tenantId, replyToken, accessToken) {
    const customersSnapshot = await db
        .collection(`tenants/${tenantId}/customers`)
        .where("lineUserId", "==", userId)
        .limit(1)
    ...
    await (0, line_1.replyMessage)(replyToken, [
        (0, line_1.createTextMessage)("配信を再開しました。\n今後もお得な情報をお届けします！"),
    ], accessToken);
}
