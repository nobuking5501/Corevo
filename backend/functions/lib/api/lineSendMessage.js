        ...
        for (const tenantDoc of tenantsSnapshot.docs) {
            ...
            for (const appointmentDoc of appointmentsSnapshot.docs) {
                try {
                    ...
                    const salonName = tenantDoc.data()?.name || "サロン";
                    // リマインダーメッセージを送信
                    await (0, line_1.pushMessage)(customerData.lineUserId, [(0, line_1.createAppointmentReminderMessage)(appointmentDate, serviceName, salonName)], lineConfig.channelAccessToken);
                    // メッセージ履歴を保存
                    await db.collection(`tenants/${tenantId}/messages`).add({
                        tenantId,
